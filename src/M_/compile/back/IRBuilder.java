...
import static M_.IR.RegisterSet.*;
...
public class IRBuilder implements IAstVisitor {
    ...
    private HashMap<FuncionSymbol,Integer> operationsCountMap;
...
//                addValueBackOptimizeCode(funcDeclaration);
        ...
        /* initStatement function used to initialize global variables */
...
//    }
    ...
    public void visit(FuncDeclaration node) {
        ...
        /* add default return */
        for(Statement statement : node.body.statements)
    ...
    public void visit(ConstExpression node) {
        ...
        switch(node.typeName) {
            case "int":
                operand = new Immediate(valueOf(node.value));
                break;
            case "null":
    ...
    public void visit(NewExpression node) {
        ...
        if(node.restDimension == 0) {
            if(node.type instanceof ClassType) {
                ...
                if(classType.name.equals("string"))
                    ...
                    FuncionSymbol fs = classType.symbol.classSymbolTable.getFunctionSymbol(classType.name);
                    if(fs == null)
                        constructor = null;
                    else
        ...
        if(node.restDimension > 0 || node.typeNode instanceof BasicTypeNode ) {
    ...
    private int countOperations(Expression expression) {
        ...
        if (expression instanceof ArrayExpression) {
            ...
            count += countOperations(((OpExpression) expression).expression);
            count += 1;
    ...
    private int countOperations(Statement statement) {
        ...
        if(statement instanceof IfStatement) {
            count += countOperations(((IfStatement) statement).ifStatement);
        ...
        } else if(statement instanceof DeclarationStm) {
            count += countOperations(((DeclarationStm) statement).declaration.init);
        } else {
            count += 1;
        }
        return count;
    ...
    private boolean deserveInline(String name) {
        if(!Config.InlineOptm) return false;
        ...
        if(!funcDeclaration.symbol.isGlobalFunction)    //  is a method
            return false;
        List<Statement> body = funcDeclaration.body.statements;
    ...
    public void visit(OpExpression node) {
        ...
        switch(node.op) {
            case " ++": case " --": {
                ...
                curBB.append(new UnaryInst(curBB, node.op.equals(" ++") ? UnaryInst.UnaryOp.INC : UnaryInst.UnaryOp.DEC, (Address) operand));
